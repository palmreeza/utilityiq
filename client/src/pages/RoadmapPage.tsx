...
const HORIZON_COLORS: Record<RoadmapHorizon, { bg: string; border: string; text: string }> = {
  ...
  "3-12 months": { bg: "oklch(0.78 0.18 75 / 0.08)", border: "rgba(226,35,42,0.20)", text: "#e2232a" },
  "12-24 months": { bg: "oklch(0.62 0.16 220 / 0.08)", border: "oklch(0.62 0.16 220 / 0.3)", text: "oklch(0.72 0.14 220)" },
  "24+ months": { bg: "oklch(0.65 0.18 145 / 0.08)", border: "oklch(0.65 0.18 145 / 0.3)", text: "#1a9e6e" },
};
      ...
      <div className="p-6 lg:p-8 animate-fade-up">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          <button onClick={() => setFilterHorizon("all")}
            ...
            style={filterHorizon === "all" ? {
              background: "#e2232a", color: "#1e3640", borderColor: "#e2232a"
            } : { background: "#f0f0f0", color: "#727272", borderColor: "#d8d8d8" }}>
        ...
        {!items || items.length === 0 ? (
          <div className="card-base p-16 text-center">
            <Map className="w-12 h-12 mx-auto mb-4" style={{ color: "#c9c9c9" }} />
              ...
              return (
                <div key={horizon}>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-xs" style={{ color: "#727272" }}>{hItems.length} items</span>
                  </div>
                  <div className="grid gap-3">
                    {hItems.map((item: any) => {
                      const pc = PRIORITY_CONFIG[item.priority as RoadmapPriority];
                      return (
                        <div key={item.id} className="card-base p-4 hover:border-red-500/20 transition-all">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              {item.description && (
                                <p className="text-xs" style={{ color: "#727272" }}>{item.description}</p>
                              )}
                              {item.rationale && (
                                <p className="text-xs mt-2 italic" style={{ color: "#727272" }}>{item.rationale}</p>
                              )}
                            </div>
                            <div className="flex flex-col items-end gap-2 flex-shrink-0">
                              <select
                                ...
                                style={{ background: "#e8e8e8", borderColor: "#d8d8d8", color: "#252525" }}>
