    ...
    <AppLayout>
      <div className="p-6 lg:p-8 animate-fade-up">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="text-xs mb-1" style={{ color: "#727272" }}>
        ...
        {isLoading ? (
          <div className="grid gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="card-base p-5 h-24 animate-pulse" style={{ background: "#f0f0f0" }} />
            ))}
          </div>
          ...
          <div className="grid gap-4">
            {assessments.map((a: any) => {
              ...
              return (
                <div key={a.id} className="card-base p-5 cursor-pointer hover:border-red-500/30 transition-all duration-200 group"
                  onClick={() => navigate(`/assessment/${a.id}/workspace`)}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="font-semibold group-hover:text-red-600 transition-colors truncate">{a.name}</h3>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${statusCfg?.className}`}>
                          {statusCfg?.label ?? a.status}
                        </span>
                      </div>
                      {a.description && (
                        <p className="text-sm mb-3 line-clamp-2" style={{ color: "#727272" }}>{a.description}</p>
                      )}
