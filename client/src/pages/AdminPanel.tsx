    ...
    return (
      <AppLayout>
        <div className="p-8 text-center">
          <Shield className="w-12 h-12 mx-auto mb-4" style={{ color: "oklch(0.55 0.22 25)" }} />
          <h2 className="font-semibold mb-2">Access Denied</h2>
          <p style={{ color: "oklch(0.60 0.01 240)" }}>Platform Owner access required.</p>
        </div>
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="p-6 lg:p-8 animate-fade-up">
        <div className="mb-8">
          ...
          <p style={{ color: "oklch(0.60 0.01 240)" }}>Platform Owner — manage organisations, users, and templates</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { icon: Building2, label: "Organisations", value: stats?.orgs ?? orgs?.length ?? 0, color: "oklch(0.78 0.18 75)" },
            { icon: Users, label: "Users", value: stats?.users ?? users?.length ?? 0, color: "oklch(0.62 0.16 220)" },
            { icon: Database, label: "Templates", value: "—", color: "oklch(0.65 0.18 145)" },
          ].map((s) => (
            <div key={s.label} className="stat-card">
              ...
              <div className="text-xs mt-1" style={{ color: "oklch(0.50 0.01 240)" }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="card-base p-5 mb-6">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => seedTemplate.mutate()} disabled={seedTemplate.isPending}
              className="gap-2 flex-shrink-0" style={{ background: "oklch(0.78 0.18 75)", color: "oklch(0.10 0.01 240)" }}>
        ...
        <div className="mb-6">
          ...
          {showCreateOrg && (
            <div className="card-base p-5 mb-4">
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "oklch(0.60 0.01 240)" }}>Organisation Name *</label>
                ...
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "oklch(0.60 0.01 240)" }}>Slug (URL-safe) *</label>
                  <Input value={orgSlug} onChange={(e) => setOrgSlug(e.target.value)}
                    placeholder="eskom-holdings" className="bg-elevated border-subtle" />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "oklch(0.60 0.01 240)" }}>Industry</label>
                  <Input value={orgIndustry} onChange={(e) => setOrgIndustry(e.target.value)}
                    className="bg-elevated border-subtle" />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1.5" style={{ color: "oklch(0.60 0.01 240)" }}>Country</label>
          ...
          <div className="grid gap-3">
            {orgs?.map((org: any) => (
              <div key={org.id} className="card-base p-4 flex items-center justify-between cursor-pointer hover:border-amber-500/30 transition-all"
            ...
            {(!orgs || orgs.length === 0) && (
              <div className="card-base p-8 text-center">
                <Building2 className="w-8 h-8 mx-auto mb-2" style={{ color: "oklch(0.35 0.01 240)" }} />
                <p className="text-sm" style={{ color: "oklch(0.60 0.01 240)" }}>No organisations yet. Create one above.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="card-base overflow-hidden">
            <div className="divide-y" style={{ borderColor: "oklch(0.16 0.012 240)" }}>
              {users?.map((u: any) => (
                <div key={u.id} className="flex items-center gap-3 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs" style={{ color: "oklch(0.50 0.01 240)" }}>{u.email ?? u.openId}</div>
                  </div>
                  <span className="text-xs px-2 py-0.5 rounded-full"
                    style={{ background: u.platformRole === "platform_owner" ? "oklch(0.78 0.18 75 / 0.15)" : "oklch(0.16 0.012 240)",
                      color: u.platformRole === "platform_owner" ? "oklch(0.88 0.16 75)" : "oklch(0.60 0.01 240)" }}>
              ...
              {(!users || users.length === 0) && (
                <div className="p-8 text-center">
                  <p className="text-sm" style={{ color: "oklch(0.60 0.01 240)" }}>No users found.</p>
                </div>
              )}
            </div>
          </div>
