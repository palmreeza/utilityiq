  ...
  return (
    <AppLayout>
      <div className="p-6 lg:p-8 animate-fade-up">
        ...
        <p className="mb-6" style={{ color: "#727272" }}>{assessment?.name}</p>

        {isLoading ? (
          <div className="space-y-2">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="h-14 rounded-xl animate-pulse" style={{ background: "#f0f0f0" }} />
            ))}
          </div>
        ) : !entries || entries.length === 0 ? (
          <div className="card-base p-12 text-center">
            <Clock className="w-10 h-10 mx-auto mb-3" style={{ color: "#c9c9c9" }} />
            <p style={{ color: "#727272" }}>No audit events recorded yet.</p>
          </div>
        ) : (
              ...
              {entries.map((entry: any) => (
                <div key={entry.id} className="flex items-start gap-4 p-4 hover:bg-elevated transition-colors">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                    ...
                    <Clock className="w-4 h-4" style={{ color: "#e2232a" }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-sm font-medium font-mono" style={{ color: "#e2232a" }}>
                        {entry.action}
                      </span>
                    </div>
                    <div className="flex items-center gap-3 text-xs" style={{ color: "#727272" }}>
                    ...
                    {entry.metadata && Object.keys(entry.metadata).length > 0 && (
                      <div className="mt-1 text-xs font-mono px-2 py-1 rounded" style={{ background: "#f0f0f0", color: "#727272" }}>
